...
- MoonBit 源码规模：项目包含 36 个受 Git 跟踪的 MoonBit 源文件，共 4182 行，其中 23 个实现文件（3503 行）、13 个测试文件（679 行）。
- 来源说明：所有实现与说明均按 MoonGeoRoute 主题整理。
- CI：已提供官方风格的 `test.yml` 与 `publish.yml`；`test.yml` 对 native、wasm、wasm-gc 和 JS 执行严格矩阵检查。
...
- Mooncakes：`phjphj676/moongeoroute@0.1.6` 已完成发布，发布前校验和发布包复检均通过。

## Verified release-candidate evidence

This section supersedes earlier provisional counts and release notes in this file.

- Candidate version: `0.1.6` in `moon.mod`.
- Effective MoonBit source: 36 tracked `.mbt` files and 4182 lines, excluding build output.
- Test gate: 47 passed, 0 failed with `moon test --deny-warn --target all`.
...
- Mooncakes publication was verified by a successful `moon publish` response for the exact `0.1.6` package; the packaged copy also passed `moon check`.
